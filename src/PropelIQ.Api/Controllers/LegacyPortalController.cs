    ...
    public async Task<IActionResult> GetStaffQueue([FromQuery] string? date, CancellationToken ct = default)
    {
        ...
        var queue = queueResult.Items.Select(a =>
        {
            ...
            return new
            {
                ...
                patient_first_name = string.IsNullOrWhiteSpace(first) ? null : first,
                patient_last_name = string.IsNullOrWhiteSpace(last) ? null : last,
                ...
                status = a.Status,
                checkout_status = a.Status == "arrived" ? "confirmed" : null,
